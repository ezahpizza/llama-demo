...
def init():
    global pinecone_index, vector_store, parser
    ...
    try:
        ...
        if not validate_multimodal_setup():
            raise ValueError("Multimodal processing setup validation failed")
...
async def query_documents(request: QueryRequest):
    try:
        ...
        # query engine multimodal content
        query_engine = index.as_query_engine(
            similarity_top_k=15,  # Increased for multimodal content
        ...
        # multimodal context
        mm_query = f"""
        Based on the available text content and image analysis from the documents, please answer:
        {request.query_text}
        
        Please consider both textual information and any charts, graphs, tables, or visual data 
        that may contain relevant information. If you find data in charts or graphs, please 
        reference the specific visual elements and their data points.
        """
        
        response = query_engine.query(mm_query)
            ...
            for node in response.source_nodes:
                ...
                source_info = {
                    ...
                    "metadata": {
                        ...
                        "parsing_method": node.metadata.get("parsing_method", "unknown"),
                        "confidence_score": node.metadata.get("confidence_score"),
                        "image_description": node.metadata.get("image_description"),
...
async def health_check():
    """Health check with multimodal status"""
